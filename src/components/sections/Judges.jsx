...
export default function Judges() {
  ...
  useEffect(() => {
    async function loadJudges() {
      try {
        const data = await fetchSanityData(queries.judges);
        if (data && data.length > 0) {
          setJudges(
            data.map((judge) => ({
              ...
              image: judge.image || '/speaker-1.png',
            }))
          );
        }
      } catch (error) {
  ...
  if (!loading && judges.length === 0) return null;

  const totalPages = Math.ceil(judges.length / PAGE_SIZE);
  const visibleJudges = judges.slice(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE);
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            Meet the distinguished panel of experts evaluating Nigeria's most innovative space and technology startups.
          </p>
        </motion.div>
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-400">Loading judges...</p>
          </div>
        ) : (
          <motion.div
            key={page}
              ...
              >
                <div className="relative bg-dark-100 rounded-2xl overflow-hidden border border-white/5 hover:border-primary/30 transition-all duration-300">
                  ...
                  <div className="p-5">
                    ...
                    {judge.title && (
                      <p className="text-primary text-sm font-medium mb-1">{judge.title}</p>
                    )}
                    <p className="text-gray-500 text-sm">{judge.organization}</p>
                  </div>
                  {/* Hover glow */}
